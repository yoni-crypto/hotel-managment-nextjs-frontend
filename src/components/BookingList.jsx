            ...
            <table className="w-full rounded-lg mt-4">
                <thead className="bg-[#f4f3ff] rounded-xl p-2">
                    <tr>
                        ...
                        <th className=" px-4 py-3 text-left text-sm">Room Number</th>
                        <th className=" px-4 py-3 text-left text-sm">Duration</th>
                        <th className=" px-4 py-3 text-left text-sm">Check-In & Check-Out</th>
                        ...
                        <>
                            {bookingData.map((booking, index) => (
                                <tr key={index} className='border-b border-gray-200'>
                                    ...
                                    <td className="px-4 py-4 text-sm text-gray-600">{booking.roomType}</td>
                                    <td className="px-4 py-4 text-sm text-gray-600">{booking.roomNumber}</td>
                                    <td className="px-4 py-4 text-sm text-gray-600">{booking.duration}</td>
                                    <td className="px-4 py-4 text-sm text-gray-600">{booking.checkIn} - {booking.checkOut}</td>
                                    <td className="px-2 my-4 text-sm text-gray-600 font-semibold">
                                        <span className={`px-2 py-1 rounded-md  ${booking.status === "Checked-in" ? "bg-green-500 text-white" : "text-blue-500"}`}>
                                            {booking.status}
                                        </span>
                                    </td>
                                </tr>
